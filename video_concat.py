...
class VideoConcatFFmpeg:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "color_match_mode": (
                    ["None", "Match Brightness", "Match Contrast", "Match Saturation", "Match Brightness + Contrast", "Match Brightness + Saturation", "Match Contrast + Saturation", "Match All (Br. + Contr. + Sat.)"], 
                    {"default": "None"}
                ),
                "wb_gamma_mode": (
                    ["None", "Match White Balance (Soft)", "Match WB + Gamma (Experimental)"], 
                    {"default": "None"}
                ),
    ...
    # --- SECURITY HELPERS ---
    def is_input_path_allowed(self, path_str: str) -> bool:
        """Проверяет, можно ли ЧИТАТЬ из этого пути"""
        try:
            if len(path_str) > self.max_path_len: return False
            ...
            for allowed in allowed_roots:
                if hasattr(path, "is_relative_to"):
                    if path.is_relative_to(allowed): return True
                else:
                    if str(path).startswith(str(allowed)): return True
            return False
        except:
            return False

    def sanitize_output_path(self, user_path: str) -> str:
        """Очищает путь для ЗАПИСИ (только в output)"""
        root_output = Path(self.output_dir).resolve()
        
        if not user_path or user_path.strip() == "":
            return str(root_output)
        
        if len(user_path) > self.max_path_len:
            print("[VideoConcat] Path too long. Using root.")
            return str(root_output)
        ...
        try:
            ...
            if path.is_absolute():
                ...
                # Проверяем, внутри ли он root_output
                is_safe = False
                try:
                    if hasattr(resolved, "is_relative_to"):
                        if resolved.is_relative_to(root_output): is_safe = True
                    else:
                        if str(resolved).startswith(str(root_output)): is_safe = True
                except: pass
                
                if is_safe:
                    return str(resolved)
                
                # Если абсолютный путь "снаружи" -> берем только имя папки + очистка
                print(f"[VideoConcat] Security: Absolute path outside output. Using root.")
            ...
            print(f"[VideoConcat] Path error: {e}. Using root.")
            return str(root_output)

    def analyze_frame_stats(self, path):
        # (Код без изменений)
        stats = {"duration": 0.0, "has_audio": False, "r_avg": 0.0, "g_avg": 0.0, "b_avg": 0.0, "luma_avg": 0.0, "luma_std": 0.0, "sat_avg": 0.0, "valid": False}
        try:
    ...
    def concatenate_videos(self, num_VideoFile_paths, num_VideoDir_paths, output_name, output_path, 
        ...
        # 1. Output Security
        target_dir = self.sanitize_output_path(output_path)
        os.makedirs(target_dir, exist_ok=True)

        if not output_name or not output_name.strip():
            filename = f"merged-{datetime.datetime.now().strftime('%Y-%m-%d')}"
        else:
            filename = re.sub(r'[^\w\-\.]', '_', output_name.strip())
        ...
        for i in range(1, num_VideoFile_paths + 1):
            ...
            if path and isinstance(path, str) and path.strip():
                ...
                # ПРОВЕРКА СУЩЕСТВОВАНИЯ ПЕРЕД ПРОВЕРКОЙ БЕЗОПАСНОСТИ
                if os.path.exists(clean):
                    if self.is_input_path_allowed(clean):
                        video_files.append(os.path.abspath(clean))
                    else:
                        print(f"[VideoConcat] Security Block: Input file outside allowed dirs: {clean}")
                else:
                    # Просто пропускаем, если файла нет
                    pass
                    ...
                    if self.is_input_path_allowed(clean):
                        try:
                            ...
                            for f in os.listdir(clean):
                                full_path = os.path.join(clean, f)
                                if os.path.splitext(f)[1].lower() in valid_ext:
                                    if self.is_input_path_allowed(full_path):
                                        dfs.append(os.path.abspath(full_path))
                            dfs.sort()
                            video_files.extend(dfs)
                        except: pass
                    else:
                        print(f"[VideoConcat] Security Block (Dir): {clean}")
        ...
        if "Copy" in ffmpeg_mode:
            ...
            if not do_crossfade and not any_effect:
                ...
                for idx, v in enumerate(video_files):
                    ...
                    if idx == 0:
                        ref = info.copy()
                        if ref["sat_avg"] < 10: apply_sat = False
                        if ref["luma_avg"] < 15 or not ref["valid"]: apply_contr = False; apply_gamma = False
                ...
                for i in range(len(files_data)):
                    ...
                    if any_effect and i > 0 and cur["valid"]:
                        ...
                        if apply_sat:
                            ...
                            if sat < 1.0: sat = max(0.9, sat) 
                            else: sat = min(1.6, sat)
